...
Definition list_mapₛ : human_program := {[
  "list_map" := (BNamed "arg", (
    let: "fn" := ![𝟙] "arg" in
    let: "xs" := ![𝟚] "arg" in
    match: "xs" with
...
Definition list_mapₜ : human_program := {[
  "list_map" := (BNamed "arg", (
    let: "fn" := ![𝟙] "arg" in
    let: "xs" := ![𝟚] "arg" in
    match: "xs" with
...
Lemma list_map_sound :
  program_refinement
    (human_program_compile list_mapₛ)
    (human_program_compile list_mapₜ).
Proof.
  rewrite /list_mapₛ /list_mapₜ. apply tmc_sound.
  - split.
