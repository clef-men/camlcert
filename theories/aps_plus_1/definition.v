...
Inductive aps_plus_dir ξ : data_expr → data_expr → Prop :=
  ...
  | aps_plus_dir_plus_1 n eₛ eₜ :
      aps_plus_aps ξ n eₛ eₜ →
      aps_plus_dir ξ
        (n + eₛ)
        eₜ
  | aps_plus_dir_plus_2 n eₛ eₜ :
      aps_plus_aps ξ n eₛ eₜ →
      aps_plus_dir ξ
        (eₛ + n)
        eₜ
...
with aps_plus_aps ξ : data_expr → data_expr → data_expr → Prop :=
  ...
  | aps_plus_aps_let acc eₛ1 eₛ2 eₜ1 eₜ2 :
      ...
      aps_plus_aps ξ acc
        (let: eₛ1 in eₛ2)
        (let: eₜ1 in eₜ2)
  | aps_plus_aps_call acc annot func eₛ func_aps eₜ eₜ' :
      ...
      aps_plus_aps ξ acc
        ...
        eₜ'
  | aps_plus_aps_plus_1 acc n eₛ eₜ :
      aps_plus_aps ξ $0 eₛ.[ren (+1)] eₜ →
      aps_plus_aps ξ acc
        (n + eₛ)
        (let: acc + n in eₜ)
  | aps_plus_aps_plus_2 acc n eₛ eₜ :
      aps_plus_aps ξ $0 eₛ.[ren (+1)] eₜ →
      aps_plus_aps ξ acc
        (eₛ + n)
        (let: acc + n in eₜ)
